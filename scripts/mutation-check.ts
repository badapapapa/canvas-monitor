...
const MUTATIONS: Mutation[] = [
  ...
  {
    name: 'a group that refuses its files described as a concluded course (D-72)',
    file: 'src/sync/run.ts',
    from: "  if (coverage === 'none' && context.contextType === 'group') {",
    to: '  if (false) {',
    tests: [SYNC],
  },
  {
    name: 'a retired context left with an active alert for ever (D-72)',
    file: 'src/sync/run.ts',
    from: 'const retired = await retireContextAlerts(ctx.db, new Set(contexts.map((c) => c.contextId)), now);',
    to: 'const retired: string[] = [];',
    tests: [SYNC],
  },
  {
    name: 'an ops message rendered with a decision number still in it (D-72)',
    file: 'src/notify/render.ts',
    from: '  const summary = escapeHtml(withoutDecisionRefs(payload.summary));',
    to: '  const summary = escapeHtml(payload.summary);',
    tests: ['test/unit/notify.test.ts'],
  },
  {
    name: 'a decision number back in an alert text (D-72)',
    file: 'src/sync/run.ts',
    from: "'If the course has ended, files not already archived can no longer be fetched.',",
    to: "'If the course has ended, files not already archived can no longer be fetched (D-36).',",
    tests: ['test/unit/notify.test.ts'],
  },
  {
    name: 'seed-groups: a new group filed under the fresh proposal instead of the reviewed parent (D-72)',
    file: 'src/discover/merge-groups.ts',
    from: '          module_code: parent.proposed.module_code,',
    to: '          module_code: g.proposed.module_code,',
    tests: ['test/unit/merge-groups.test.ts'],
  },
  {
    name: 'seed-groups: a group left behind still enabled (D-72)',
    file: 'src/discover/merge-groups.ts',
    from: "    if (c.context_type !== 'group' || !c.proposed.enabled || freshKeys.has(key(c))) return c;",
    to: '    return c;',
    tests: ['test/unit/merge-groups.test.ts'],
  },
  {
  ...
  },
  {
    ...
    from: 'return { ok: false, authRefusal: status === 401 || status === 403, blocked: blockedReason(error), detail: describe(error) };',
    to: 'return { ok: false, authRefusal: true, blocked: blockedReason(error), detail: describe(error) };',
    tests: [VERIFY],
  },
  {
  ...
  {
    name: 'verify accepting BLOCKED without the control (a usage block would pass as read-only enforcement)',
    file: 'src/readmodel/cli.ts',
    from: "    if (!control.ok) return { pass: false, got: `blocked, but the control failed (${probe.detail})`, control: controlLine };\n",
    to: '',
    tests: [VERIFY],
  },
  {
  ...
  {
    name: 'production without Strict-Transport-Security',
    file: 'dashboard/lib/headers.ts',
    from: "...(mode.production ? { 'Strict-Transport-Security'",
    to: "...(false ? { 'Strict-Transport-Security'",
    tests: [DASHBOARD],
  },
  {
    name: 'production without upgrade-insecure-requests',
    file: 'dashboard/lib/headers.ts',
    from: "...(production ? ['upgrade-insecure-requests'] : []),",
    to: '...([] as string[]),',
    tests: [DASHBOARD],
  },
  {
    name: 'the local http preview sending upgrade-insecure-requests (Safari posts the login to https://localhost)',
    file: 'dashboard/lib/headers.ts',
    from: "...(production ? ['upgrade-insecure-requests'] : []),",
  ...
  {
    name: 'HSTS sent off the production deployment (over plain http, or on localhost)',
    file: 'dashboard/lib/headers.ts',
    from: "...(mode.production ? { 'Strict-Transport-Security'",
    to: "...(true ? { 'Strict-Transport-Security'",
    tests: [DASHBOARD],
  },
  {
    name: 'production session cookie without the __Host- prefix',
    file: 'dashboard/lib/session.ts',
    from: 'return productionDeployment(env) ? SESSION_COOKIE : LOCAL_SESSION_COOKIE;',
    to: 'return LOCAL_SESSION_COOKIE;',
    tests: [DASHBOARD],
  },
  {

 ...
 * And the recorded expiry must match the read-only token's own.
 *
...
interface FakeTurso {
  ...
  foreign: number;
  /** status for a write on a read-only token; 'accept' lets it through (the failure verify must catch) */
  readOnlyWrite: number | 'accept';
}
...
async function fakeTurso(f: FakeTurso): Promise<string> {
  const server = createServer((req, res) => {
    ...
    req.on('end', () => {
      ...
      const writes = requests.some((r) => /^\s*insert/i.test(r.stmt?.sql ?? ''));
      if (writes && role === 'ro' && f.readOnlyWrite !== 'accept') { deny(f.readOnlyWrite); return; }
      const results = requests.map((r) => r.type === 'execute'
        ? { type: 'ok', response: { type: 'execute', result: { cols: [{ name: 'value', decltype: null }], rows: [[{ type: 'text', value: '1' }]], affected_row_count: 0, last_insert_rowid: null } } }
...
const good = () => ({ tokens: {}, foreign: 401, readOnlyWrite: 401 as const });
...
describe('readmodel verify: only an authorisation refusal is a pass', () => {
  it('passes all six checks when Turso refuses with 401, and prints no token or URL', async () => {
    ...
    assert.match(r.out, /refused \(SERVER_ERROR HTTP 401\)/);
  ...
  it('accepts 403 as an authorisation refusal too', async () => {
